...
export async function POST(req) {
    ...
    if (resumeLink?.trim()) {
        systemPrompt += `\n\nAt the end, include the word “Resume” as a hyperlink pointing to: ${resumeLink.trim()}`;
    }

    // Compose user prompt with additional details
    let userPrompt = `Job Description:\n${jobDescription.trim()}`;
    if (manager) userPrompt += `\n\nHiring Manager: ${manager.trim()}`;
